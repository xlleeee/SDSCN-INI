# base_topo_1, 基本拓扑，用于网络节点及节点间连接关系
...
'''
#print(nx.has_path(G,'R15','R15'))
#print(nx.has_path(G,'H18','P17'))

# 测试无向图的连通性

for i in range(1,20): # source: RX
	source = 'R'+str(i)
	for j in range(1,20): # target: RX
		target = 'R'+str(j)
		if nx.has_path(G,source,target)==False:
			print(source)
			print(target)
	
	for j in range(1,11): # target: RX
		target = 'R'+str(j)
		if nx.has_path(G,source,target)==False:
			print(source)
			print(target)
